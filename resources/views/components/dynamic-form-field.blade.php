...
@endif
    ...
    @elseif($type === 'checkbox-group')
        ...
        @php
            ...
            $finalValue = !is_null($oldValue) ? $oldValue : $dataValue; 
            $jsonValue = json_encode(is_array($finalValue) ? $finalValue : []);
            /* 
                (is_array(old($name)) 
                    ? old($name) :
                    (is_array($data) ? $data : []) ) 
        ...
        <div {!! $alpineDirectives !!}
            x-data="{
                selectedValues: {{ $jsonValue  /* json_encode(is_array($data) ? $data : []) */ }},
